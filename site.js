...
/* ============================================================
   Animated spirometry manoeuvre demonstration
   ============================================================ */
(function () {
  var wrap = document.getElementById('spiroDemo');
  if (!wrap) return;
  var path = document.getElementById('spiroPath');
  var dot = document.getElementById('spiroDot');
  var btn = wrap.querySelector('[data-demo-play]');
  var label = wrap.querySelector('[data-demo-label]');
  var cue = wrap.querySelector('[data-demo-cue]');
  var clock = wrap.querySelector('[data-demo-clock]');
  var len = path.getTotalLength();
  var DUR = 9000;
  var raf = null, t0 = 0;

  var CUES = [
    [0.00, 'Settle', 'Two or three quiet breaths on the mouthpiece. Check the seal and the nose clip.', '&ldquo;Just breathe normally for me.&rdquo;'],
    [0.22, 'Inspire to TLC', 'A rapid, complete breath in. Pause less than two seconds at the top.', '&ldquo;Now the biggest breath you can &mdash; all the way in, more, more.&rdquo;'],
    [0.36, 'Blast', 'Maximal force from the very first instant. No hesitation, or the BEV fails.', '&ldquo;BLAST it out &mdash; hard and fast!&rdquo;'],
    [0.50, 'Keep going', 'Past the peak the flow is effort independent, but the volume is not. Do not let them stop.', '&ldquo;Keep going, keep going, squeeze it all out.&rdquo;'],
    [0.70, 'Reach the plateau', 'Continue to a 1-second plateau with less than 0.025 L expired, or 15 seconds.', '&ldquo;Don&rsquo;t stop &mdash; I&rsquo;ll tell you when.&rdquo;'],
    [0.88, 'Inspire back to TLC', 'Closes the loop and proves they started from full inflation.', '&ldquo;And a big breath back in, fast, all the way.&rdquo;']
  ];
  ...
  function paint(p) {
    ...
    var c = CUES[0];
    for (var i = 0; i < CUES.length; i++) if (p >= CUES[i][0]) c = CUES[i];
    cue.innerHTML = '<b>' + c[1] + '</b>' + c[2] + ' <span class="say">' + c[3] + '</span>';
    clock.textContent = (p * 9).toFixed(1) + ' s';
  }

  function stop() {
    if (raf) cancelAnimationFrame(raf);
    raf = null; btn.removeAttribute('data-running'); label.textContent = 'Watch again';
  }
  ...
  function play() {
    if (raf) { stop(); return; }
    t0 = 0; btn.setAttribute('data-running', '1'); label.textContent = 'Stop';
  ...
  paint(0);
  path.style.strokeDashoffset = 0;   /* show the full trace until first play */
})();
